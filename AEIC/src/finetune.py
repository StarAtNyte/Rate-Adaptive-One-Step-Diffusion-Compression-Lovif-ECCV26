...
def main(args):
    ...
    class FolderDataset(_DS):
        def __init__(self, root, transform=None):
            import glob as _g
            self.files = sorted(f for r in root.split(",") for f in _g.glob(_os.path.join(r, "**", "*.png"), recursive=True))
            self.transform = transform
        def __len__(self):
            return len(self.files)
        def __getitem__(self, idx):
            import numpy as _np
            img = _Im.open(self.files[idx]).convert("RGB")
            w, h = img.size
            if min(w, h) < 512:  # ponytail: upscale rare small images instead of filtering
                sc = 512 / min(w, h)
                img = img.resize((int(w * sc) + 1, int(h * sc) + 1))
            arr = _np.asarray(img)
            return self.transform(arr) if self.transform else arr

    DatasetCls = H5Dataset if args.train_dataset_2K.endswith(".hdf5") else FolderDataset
    ...
    net_disc = vision_aided_loss.Discriminator(cv_type='dino', output_type='conv_multi_level', loss_type="multilevel_sigmoid_s", device="cuda")
    ...
    net_lpips = lpips.LPIPS(net='alex').to(accelerator.device)
    ...
    while global_step <= cfg["max_train_steps"]:
        for batch in train_dataloader:
            ...
            with accelerator.accumulate(*l_acc):
                ...
                loss_lpips_t = net_lpips(x_hat.clamp(-1, 1), x.clamp(-1, 1)).mean()
                ...
                loss = RateLossOutput.rate_loss * current_hp["lambda_rate"] \
                     ...
                     + loss_lpips_t * float(current_hp.get("lambda_lpips", 0.0))
